...
class Fun(commands.Cog):
    ...
    @discord.app_commands.command(
        description="Get a random joke from people with good humour"
    )
    async def joke(self, interaction: discord.Interaction):
        ...
        if joke["type"] == "twopart":
            await interaction.response.send_message(
                embed=discord.Embed(
                    title=joke["setup"], description=f"{joke['delivery']}"
                )
            )
        else:
    ...
    async def meme(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            embed=discord.Embed(title=meme["title"]).set_image(url=meme["preview"][-1])
        )

    @discord.app_commands.command(
        description="Search a term in the Urban Dictionary (srsly wtf)"
    )
    async def urbandictionary(self, interaction: discord.Interaction, term: str):
        resp = await request_helper.get(
            f"https://unofficialurbandictionaryapi.com/api/search?term={term}&strict=false&matchCase=false&limit=5&page=1&multiPage=false"
        )
    ...
    @commands.hybrid_group(
        invoke_without_command=True, description="Convert a number into binary"
    )
